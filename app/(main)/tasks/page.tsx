    ...
    const fetchTasks = useCallback(async () => {
        try {
            ...
            if (res.ok) {
                ...
                // ★完了(DONE)タスクのみをフィルタリング
                const completedTasks = data.filter(t => t.status === "DONE");

                // 日付順にソート（完了済みなので新しい順が良い場合もありますが、一旦ダッシュボードと合わせます）
            ...
            <div className="pt-24 px-6 max-w-5xl mx-auto pb-20">
                ...
                {isLoading ? (
                    ...
                    <div className="flex overflow-x-auto grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 place-items-center">
                        {tasks.map((task) => (
                            <div key={task.id} className="flex-shrink-0 opacity-80 hover:opacity-100 transition-opacity">
                                <TaskCard
                                    id={task.id}
                                    title={task.title}
